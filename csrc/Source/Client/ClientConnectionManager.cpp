...
ClientConnectionManager::ClientConnectionManager(const std::wstring& gameName, IDiscovererPtr discoverer) :
	mDiscoverer(discoverer),
	mPolicy(new DefaultUserManagerPolicy()),
	mGameName(gameName),
	mReconnecting(false),
	mConnected(false),
	mKeyTimeoutMS(0),
...
ClientConnectionManager::ClientConnectionManager(const std::wstring& gameName, IDiscovererPtr discoverer, IUserManagerPolicyPtr policy) :
	mDiscoverer(discoverer),
	mPolicy(policy),
	mGameName(gameName),
	mReconnecting(false),
...
void ClientConnectionManager::SetListener(IClientConnectionManagerListenerPtr clientConnectionManagerListener)
{
...
std::vector<std::wstring> ClientConnectionManager::GetUserManagerClassNames()
{
	...
	IDiscoveredGamePtr game = discoverGame(mGameName);
	if (game == NULL)
		return names;

	std::vector<IDiscoveredUserManagerPtr> userManagers = game->GetUserManagers();
...
bool ClientConnectionManager::Connect(const std::wstring& userManagerClassName, int connectAttempts, int msBetweenAttempts)
{
	...
	IUserManagerClientPtr userManager;
	try 
	{
		// MSED - Provide a real mapping between User Manager class names and DLLs
		if (userManagerClassName == L"com.sun.gi.comm.users.client.impl.TCPIPUserManagerClient")
			userManager.reset(new TCPIPUserManagerClient());
	}
...
void ClientConnectionManager::SendValidationResponse(const std::vector<ICallbackPtr>& callbacks)
{
...
void ClientConnectionManager::OnValidationDataRequest(const std::vector<ICallbackPtr>& callbacks)
{
...
{
	boost::shared_ptr<ClientConnectionManager> pThis(this, null_deleter());

	ClientChannelPtr clientChannel(new ClientChannel(pThis, name, channelID));
...
{
	ClientChannelPtr clientChannel = mChannelMap[channelID];
...
IDiscoveredGamePtr ClientConnectionManager::discoverGame(const std::wstring& gameName)
{
	std::vector<IDiscoveredGamePtr> games = mDiscoverer->GetGames();
	for (size_t i = 0; i < games.size(); ++i)
		if (games[i]->GetName() == gameName)
			return games[i];

	Platform::Log("Discovery Error: No games discovered!\n");
	return IDiscoveredGamePtr();
}

bool ClientConnectionManager::connect(IUserManagerClientPtr userManager)
{
	...
	IDiscoveredUserManagerPtr chosenUserManager = mPolicy->Choose(game, userManager->GetClassName());

	boost::shared_ptr<IUserManagerClientListener> pThis(this, null_deleter());
	return userManager->Connect(chosenUserManager, pThis);
}
