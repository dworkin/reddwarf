...
/**
 * An implementation of {@code MethodRequest} that uses op-codes to represent
 * the method names.
 *
 * @see com.sun.sgs.benchmark.shared.MethodRequest
 * @see com.sun.sgs.benchmark.shared.impl.CodeMethodRequestOp
 */
public class CodeMethodRequest implements MethodRequest {

    private static final long serialVersionUID = 0x7CABDAA;
    
    /** 
     * Note: ridiculously, the size of the variable name seems to impact the
     * size of the serialized object, so these variable names are mapped between
     * 1 letter "hard to read" versions and more normal versions.
     */
    
    /** The arguments to the method in {@code byte} form. */
    private transient byte[] args;
    private byte[] a;

    /** Op-code of the name of the method to be invoked. */
    private transient byte opCode;
    private byte o;
    ...
    private transient boolean hasObjectArgs;
    private boolean h;
    
    /**
     * Constructs a method request with the provided method op-code and
     * an empty list of arguments.
     *
     * @param opCode the op-code of the method to be invoked
     *
     * @throws NullPointerException if {@code opCode} is null
     */
    public CodeMethodRequest(CodeMethodRequestOp opCode) {
	args = null;
        hasObjectArgs = false;
     ...
     *                be invoked.
     *
     * @throws NullPointerException if {@code opCode} or {@code args} are null
     */
    public CodeMethodRequest(CodeMethodRequestOp opCode, byte[] args) {
     ...
     * @throws NullPointerException if {@code opCode} is null
     */
    public CodeMethodRequest(CodeMethodRequestOp opCode, Object[] args) {
        if (args == null) throw new NullPointerException();
     ...
     * {@inheritDoc}
     *
     * @throws IllegalArgumentException if the opCode that the {@code
     *         MethodRequest} was created with cannot be mapped to a method
     *          name.
     */
    public String getMethodName() {
        mapVarsToVerbose();
        CodeMethodRequestOp op = CodeMethodRequestOp.fromOpCode(opCode);
        return op.getMethodName();
    }
    ...
    private void mapVarsToSuccinct() {
        a = args;
        o = opCode;
        h = hasObjectArgs;
    }
    
    private void mapVarsToVerbose() {
        args = a;
        opCode = o;
        hasObjectArgs = h;
    }
