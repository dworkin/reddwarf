...
import com.sun.sgs.impl.nio.AsyncSSLServerSocketChannelImpl;
import com.sun.sgs.impl.nio.AsyncSSLSocketChannelImpl;
import com.sun.sgs.impl.nio.ReactiveAsyncChannelProvider;
 ...
 * </dl> <p>
 */
public class TcpSecureTransport implements Transport {
    ...
    // The port for secure connections
    public static final int SECURE_PORT = 443;

    /** The listen address. */
    final InetSocketAddress listenAddress;

    // The listen address for the secure port
    final InetSocketAddress listenAddresssecure;
    ...
    // The acceptor for listening for new secure connections
    volatile AsynchronousServerSocketChannel acceptorSecure;

    /** The currently-active accept operation, or {@code null} if none. */
    volatile IoFuture<?, ?> acceptFuture = null;

    //
    volatile IoFuture<?, ?> acceptFuturesecure = null;

    /** The acceptor listener. */
    private AcceptorListener acceptorListener = null;

    // The secure acceptor listener
    private SecureAcceptorListener secureacceptorListener = null;
     ...
     */
    public TcpSecureTransport(Properties properties) {
        ...
        try {
            ...
            listenAddresssecure =
                    host == null ?
                        ...
                        new InetSocketAddress(host, SECURE_PORT);
	...
	} else if (!acceptor.isOpen() || !acceptorSecure.isOpen()) {
	    throw new IllegalStateException("transport has been shutdown");
	}
	
	if (acceptorListener != null || secureacceptorListener != null) {
	    throw new IllegalStateException("accept already called");
	}
	acceptorListener = new AcceptorListener(handler);
        secureacceptorListener = new SecureAcceptorListener(handler);
	
        assert acceptFuture == null;
        assert acceptFuturesecure == null;
        acceptFuture = acceptor.accept(acceptorListener);
        acceptFuturesecure = acceptorSecure.accept(secureacceptorListener);
	...
	final IoFuture<?, ?> future = acceptFuture;
        final IoFuture<?, ?> futuresecure = acceptFuturesecure;
	acceptFuture = null;
        acceptFuturesecure = null;
        
	if (future != null) {
	    future.cancel(true);
	}

        if (futuresecure != null) {
            futuresecure.cancel(true);
        }
	...
	if (acceptor != null && acceptor.isOpen()) {
        ...
        if (acceptorSecure != null && acceptorSecure.isOpen()) {
	    try {
		acceptorSecure.close();
            } catch (IOException e) {
                logger.logThrow(Level.FINEST, e, "closing acceptorSecure throws");
                // swallow exception
            }
	}
    ...
    private class SecureAcceptorListener
        implements CompletionHandler<AsynchronousSocketChannel, Void>
    {
        /** The connection handler. */
	private final ConnectionHandler connectionHandler;
	 ...
	 * @param connectionHandler a connection handler
	 */
	SecureAcceptorListener(ConnectionHandler connectionHandler) {
	    this.connectionHandler = connectionHandler;
	}
        ...
        public void completed(IoFuture<AsynchronousSocketChannel, Void> result)
        {
            try {
                try {
                    ...
                    // Resume accepting connections
                    acceptFuturesecure = acceptorSecure.accept(this);
                ...
                logger.logThrow(Level.SEVERE, e,
                                "acceptor error on {0}", listenAddresssecure);
                try {
                    restart();
