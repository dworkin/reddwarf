 ...
 * greater than {@code 65535}. The value {@code 0} can only be specified if
 * the {@code com.sun.sgs.impl.service.nodemap.start.server} property is
 * {@code true}, and means that an anonymous port will be chosen for running
 * the server.
 * <p>
 * <dt> <i>Property:</i> <code><b>
 ...
 * NodeAssignPolicy}, used for the node assignment policy. The class should be
 * public, not abstract, and should provide a public constructor with
 * {@link Properties} and {@link NodeMappingServerImpl} parameters.
 * <p>
 * <dt> <i>Property:</i> <code><b>
 ...
 * <dd style="padding-top: .5em"> The mimimum time, in milliseconds, that this
 * server will wait before removing a potentially inactive identity from the
 * map. This value must be greater than {@code 0}. Shorter expiration times
 * cause the map to be cleaned up more frequently, potentially causing more
 * {@link NodeMappingService#assignNode(Class, Identity) assignNode} calls;
 * longer expiration times will increase the chance that an identity will
 * become active again before it can be removed.
 * <p>
 * </dl>
 * <p>
 * This class uses the {@link Logger} named
 * <code>com.sun.sgs.impl.service.nodemap.server</code> to log information
...
public final class NodeMappingServerImpl extends AbstractService implements
    ...
    /**
     * The policy for assigning new nodes. This will likely morph into the
     * load balancing policy, as well.
     */
    ...
    /**
     * Identities waiting to be removed, with the time they were entered in
     * the map. TODO For failover, will need to persist these identities.
     */
     ...
     * change in the map. TODO For failover, these need to be persisted if we
     * have a way to reconnect an existing service to a new server
     */
     ...
     * when {@link NodeMappingServiceImpl#ready} is called), because this
	...
	String policyClassName =
		wrappedProps.getProperty(ASSIGN_POLICY_CLASS_PROPERTY);
	if (policyClassName == null) {
	    ...
	    assignPolicy =
		    wrappedProps.getClassInstanceProperty(
			    ...
			    NodeAssignPolicy.class, new Class[] {
				    Properties.class,
	...
	// from the map.
	long removeExpireTime =
		wrappedProps.getLongProperty(REMOVE_EXPIRE_PROPERTY,
			DEFAULT_REMOVE_EXPIRE_TIME, 1, Long.MAX_VALUE);
	...
	if (requestedPort == 0) {
	    logger.log(Level.CONFIG, "Server is using port {0,number,#}",
		    port);
	}

	fullName =
		"NodeMappingServiceImpl[host:" +
    ...
    public void assignNode(Class service, Identity identity,
	    long requestingNode) throws IOException {
	callStarted();
	try {
	    if (identity == null) {
	    ...
	    try {
		long newNodeId =
			mapToNewNode(identity, serviceName, node,
				requestingNode);
		logger.log(Level.FINEST, "assignNode id:{0} to {1}",
			identity, newNodeId);
     ...
     * thrash removing and adding an identity), the data store is checked to
     * see if the identity can still be removed. A service could have called
     * {@link #NodeMappingService.setStatus setStatus} during our waiting
     * time, marking the identity as active, during the waiting time. If it is
     * still appropriate to remove the identity, all traces of it are removed
     * from the data store.
     */
     ...
     * remove an identity, and, if so, removes the service bindings and
     * object.
     */
	...
	// set if dead == true; tells us the node the identity was removed
	// from
	private Node node;
	...
	/**
	 * Returns the node the identity was removed from, which can be null
	 * if the node has failed and been removed from the data store.
	 */
	Node getNode() {
	    return node;
	}
     ...
     * take a while) and then update the map to reflect the choice, cleaning
     * up old mappings as appropriate. If given a {@code serviceName}, the
     * status of the identity is set to active for that service on the new
     * node.
     * 
    ...
    long mapToNewNode(Identity id, String serviceName, Node old,
	    long requestingNode) throws NoNodesAvailableException {
	assert (id != null);
	...
	final String oldNodeKey =
		(oldNode == null) ? null : NodeMapUtil.getNodeKey(oldNode
			.getId(), id);
	final String oldStatusKey =
		(oldNode == null) ? null : NodeMapUtil.getPartialStatusKey(
			id, oldNode.getId());

	final String newNodekey = NodeMapUtil.getNodeKey(newNodeId, id);
	final String newStatuskey =
		(serviceName == null) ? null : NodeMapUtil.getStatusKey(id,
			newNodeId, serviceName);
	...
	try {
		...
		public void run() {
		    ...
		    if (oldNode != null) {
			try {
			    // Find the old IdentityMO, with the old node
			    // info.
			    IdentityMO oldidmo =
				    (IdentityMO) dataService
			    ...
			    // Remove the old IdentityMO with the old node
			    // info.
	...
	/**
	 * Returns the node found by the watchdog service, or null if this
	 * task has not run or the node has failed and been removed from the
	 * data store.
	 */
	public Node getNode() {
	    return node;
    ...
    /**
     * The listener registered with the watchdog service. These methods will
     * be notified if a node starts or stops.
     */
	...
	public void nodeFailed(Node node) {
		...
		try {
			...
			try {
			    ...
			    // This can be thrown from mapToNewNode if there
			    // are
			    ...
			    // somewhere of failed nodes, and have a
			    // background
    ...
    /**
     * Task to support node failure, run under a transaction. Finds an
     * identity that was on the failed node. Code outside the transaction
     * moves the identity to another node and removes the old id<->failedNode
     * mapping, and any status information.
     */
    ...
    /**
     * Add a node. This is useful for server testing, when we haven't
     * instantiated a service.
     * <p>
    ...
    /**
     * Task to assert some invariants about our use of the data store are
     * true. Assumes that we are in a transaction.
     */
