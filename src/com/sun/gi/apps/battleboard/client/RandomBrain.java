...
public class RandomBrain implements BattleBrain {
	...
	do {
	    i = (int)(Math.random()*boards.length);
	    if (boards[i] == null) {
		continue;
	    }
	    x = (int)(Math.random()*boards[i].getWidth());
	    y = (int)(Math.random()*boards[i].getHeight());
	    pv = boards[i].getBoardPosition(x, y);
	} while (pv != BattleBoard.PositionValue.UNKNOWN &&
		pv != BattleBoard.PositionValue.VACANT &&
		pv != BattleBoard.PositionValue.CITY);

	return new String[] {
	    boards[i].getPlayerName(),
		String.valueOf(x),
		String.valueOf(y)
	};
    }
