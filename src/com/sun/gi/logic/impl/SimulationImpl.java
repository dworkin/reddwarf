...
public class SimulationImpl implements Simulation {
	...
	public SimulationImpl(SimKernel kernel, ObjectStore ostore, Router router,
		...
		if (bootClassName != null) { // has server side
			try {
				...
				Transaction trans = ostore.newTransaction(bootclass
						.getClassLoader());
				...
				if (bootObjectID == ObjectStore.INVALID_ID) {
					...
					bootObjectID = trans.create((GLO) bootclass.newInstance(),
							"BOOT");
					if (bootObjectID == ObjectStore.INVALID_ID) { // we lost a
						// create
						// race
	...
	protected void fireChannelDataPacket(ChannelID cid, UserID from,
		...
		if (listeners != null) {
			...
			if ((channels == null) || !channels.contains(cid)) { // we arent
				// evesdropping
				return;
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.sun.gi.logic.Simulation#enableEvesdropping(com.sun.gi.comm.routing.UserID,
	 *      com.sun.gi.comm.routing.ChannelID, boolean)
	 */
