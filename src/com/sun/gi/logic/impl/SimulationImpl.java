...
public class SimulationImpl implements Simulation {
	...
	if (listeners != null) {
		...
		for (Long uid : listeners) {
		    queueTask(newTask(uid.longValue(), m,
			new Object[] { cid, from, outBuff.duplicate() }));
		}
	    } catch (SecurityException e) {
		e.printStackTrace();
	    } catch (NoSuchMethodException e) {
     ...
     * The returned ID can be used for future communication with the socket
     * that will be opened. The socket ID will not be valid, and therefore
     * should not be used until the connection is complete. Connection is
     * complete once the SimRawSocketListener.socketOpened() call back is
     * called.
     * 
