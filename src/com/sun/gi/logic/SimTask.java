...
public abstract class SimTask {
	...
	/**
	 * Registers a GLO as a listener to data packet arrival events. It listens
	 * for data adressed to the UserID passed.
	 * 
	 * The listening GLO must implement the SimUserDataListener interface.
	 * 
	 * @param id
	 *            The UserID that data will be adressed to to trigger this
	 *            listener
	 * 
	 * @param ref
	 *            A reference to the GLO to be registered
	 ...
	 * 
	 * @param simObject
	 *            the "template object" to insert into the object store, if a
	 *            GLO does not already exist with the given name. This object
	 *            should *not* be used after being passed to the createGLO call --
	 *            instead, call get() on the returned GLOReference to get the
	 *            newly created object.
	 * 
	 * @param name
	 *            an optional symbolic reference to assign to the object, or
	 *            null to create an anonymous GLO.
	 * 
	 ...
	 * 
	 * @param simObject
	 *            the "template object" to insert into the object store. This
	 *            object should *not* be used after being passed to the
	 *            createGLO call -- instead, call get() on the returned
	...
	/**
	 * This method is used to retrieve an GLReference based on the symbolic name
	 * assigned to the GLO at the time of creation in the objectstore.
	 * 
	 * As is everything else is the obejctstore, symbolic names are specific to
	 * a game context. (The Game's App ID is an implicit part of the key.)
	 * 
	 * @param gloName
	 ...
	 * 
	 * @param ref
	 *            A GLOReference to the GLO to be destroyed.
	 */
	public abstract void destroyGLO(GLOReference<? extends GLO> ref);

	/**
	 * This method opens a comm channel and returns an ID for it. If the channel
	 * is already open, return a valid ChannelID for the existing channel. Note
	 * that channels may be referred to by more than one channelID.
	 * 
	 * @param channelName
	 *            the name of the channel to create
	 * 
	 * @return a new ChannelID for the channel created. ChannelIDs are not
	 *         unique -- a channel may be referred to by more than one
	 *         channelID.
	 */
	public abstract ChannelID openChannel(String channelName);

	/**
	 * This function is used to schedule a Task to be launched on a stack-local
	 * heartbat timer. This version of the call schedules the Task to be
	 * launched with exclusive access to the called back GLO. (a "get")
	 * 
	 * It is a primative function that in of itself will not scale. APplications
	 * should generally use theincluded Persistant Destributed Timer utility
	 * (which itself uses this) PDTimer.
	 * 
	 * @see PDTimer
	 * 
	 * @param delay
	 *            Amount of time in seconds between heartbeat callbacks
	 * @param repeat
	 *            Should this repeat every delay-seconds or is it a one-shot?
	 * @param ref
	 *            The SimTimerListener to star the Task with.
	 * 
	 ...
	 * 
	 * @param access
	 *            What kind of access (get/peek/attempt) the Task will use to
	 *            acquire the GLO referred to by ref
	 * @param delay
	 *            Amount of time in seconds between heartbeat callbacks
	 * @param repeat
	 *            Should this repeat every delay-seconds or is it a one-shot?
	...
	/**
	 * This method makes a GLORefrence for a GLO. It is used to get a GLO
	 * refernce when all you have is a Java reference to the GLO in question.
	 * 
	 * Note that this only makes sense from inside a Task that has acquired the
	 * GLO through lock, peek or attempt. Use elsewhere will result in an
	 * InstantiationException.
	 * 
	 * @param glo
	 *            The GLO to make a GLOreference to.
	 * 
	 * @return a GLOReference that points to the given GLO.
	 * 
	 * @throws InstantiationException
	 *             if the attempt to make a GLOReference fails.
	 */
	public abstract <T extends GLO> GLOReference<T> makeReference(T glo)
			throws InstantiationException;

	/**
	 * THIS IS AN INTERNAL FUNCTION AND SHOULD NEVER BE CALLED BY SGS
	 * APPLICATION CODE!
	 * 
	 * It is used by the system as GLOs are acquired to store data about them in
	 * the SimTask for use by other methods.
	 * 
	 * @param objID
	 *            The ID of the object
	 * @param glo
	 *            A pointer to the GLO itself
	 * @param access
	 *            The access type used to acquire the object
	 */
	public abstract void registerGLOID(long objID, GLO glo, ACCESS_TYPE access);

	/**
	 * This method is used to queue a child task for execution. A child task
	 * will *never* be started til after the parent-task has finished
	 * processing.
	 * 
	 * @param accessType
	 *            Access type used to acquire the initial GLO
	 * @param target
	 *            The Task's initial GLO
	 * @param method
	 *            The method to call upon that GLo to start the Task
	 * @param parameters
	 *            Parameters to pass in that starting call.
	 */
	...
	/**
	 * This is a conveneince method, the same as queueTask above but without an
	 * Access type parameter. GET access is assuemd if you use this call.
	 * 
	 * @see queueTask(ACCESS_TYPE accessType, GLOReference<? extends GLO>
	 *      target, Method method, Object[] parameters);
	 * 
	 * @param target
	 * @param method
	 * @param parameters
	...
	/**
	 * GLC code is often written with an assumption about the access type that
	 * the containg GLO will be acquired with. For instance, a method that
	 * modifies a GLO's state may not want to be called in Access Type PEEK
	 * because then the changes will not be saved.
	 * 
	 * Thsi method provides a way for the GLC code to enforce the access type.
	 * If the access type passed in with thsi call is *not* the current access
	 * type of the passed GLO, this method will throw an
	 * AccessTypeViolationException which will result in the run of the Task
	 * being aborted if not caught by the surrounding GLC code.
	 * 
	 * 
	 * @param accessType
	 *            The required Access Type
	 * @param glo
	 *            The GLO to check the Access Type of
	...
	/**
	 * Requests that a socket be opened at the given host on the given port. The
	 * returned ID can be used for future communication with the socket that
	 * will be opened. The socket ID will not be valid, and therefore should not
	 * be used until the connection is complete. Connection is complete once the
	 * SimRawSocketListener.socketOpened() call back is called.
	 * 
	 * @param access
	 *            the access type (GET, PEEK, or ATTEMPT)
	 * @param ref
	 *            a reference to the GLO initiating the connection
	 * @param host
	 *            a String representation of the remote host
	 * @param port
	 *            the remote port
	 * @param reliable
	 *            if true, the connection will use a reliable protocol
	 * 
	 * @return an identifier that can be used for future communication with the
	 *         socket.
	 */
	...
	/**
	 * Sends data on the socket mapped to the given socketID. This method will
	 * not return until the entire buffer has been drained.
	 * 
	 * @param socketID
	 *            the socket identifier.
	 * @param data
	 *            the data to send. The buffer should be in a ready state, i.e.
	 *            flipped if necessary.
	 */
	public abstract void sendRawSocketData(long socketID, ByteBuffer data);

	/**
	 * Requests that the socket matching the given socketID be closed. The
	 * socket should not be assumed to be closed, however, until the call back
	 * SimRawSocketListener.socketClosed() is called.
	 * 
	 * @param socketID
	 ...
	 *             if called outside of the context of an SGS task dispatch.
	 */
	 ...
	 * APPLICATION CODE!
	 * 
	 * It is used by the system to set the SimTask returned by
	 * SimTask.getCurrent();
	 * 
	 * 
	 */
