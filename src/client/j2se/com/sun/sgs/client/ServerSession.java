 ...
 * session.  A client can use its <code>ServerSession</code> to send
 * messages to the server, to check if it is connected, or to log out.
 *
 * <p>A server session has an associated {@link ServerSessionListener}
 * that is notified of session communication events such as message
 * receipt, channel joins, reconnection, or disconnection.
 *
 * Once a server session is disconnected, it can no longer
...
public interface ServerSession {
     ...
     * server.  The specified message is sent asychronously to the
     * server; therefore, a successful invocation of this method does
     * not indicate that the given message was successfully sent.
     * Messages that are received by the server are delivered in
     * sending order.
     *
     ...
     * that the client logged out.  When a session has been logged
     ...
     * ServerSessionListener} passing a <code>boolean</code>
     * indicating whether the disconnection was graceful.
     *
     * <p>If this server session is already disconnected, then no
     * action is taken.
     *
