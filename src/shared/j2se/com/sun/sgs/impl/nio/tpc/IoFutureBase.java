...
class IoFutureBase<R, B, A extends B>
    implements IoFuture<R, A>, RunnableFuture<R>
{
    static <R, B, A extends B> IoFutureBase<R, B, A>
        create(Runnable runner, A attachment, CompletionHandler<R, B> handler)
    {
        return new IoFutureBase<R, B, A>(runner, attachment, handler);
    }
    ...
    protected IoFutureBase(Runnable runner,
            A attachment,
    ...
    void notifyCompletionHandler() {
        handler.completed(new IoFuture<R, B>() {
            ...
            public boolean cancel(boolean mayInterruptIfRunning) {
                return IoFutureBase.this.cancel(mayInterruptIfRunning);
            }

            public R getNow() throws ExecutionException {
                return IoFutureBase.this.getNow();
            }

            public R get() throws InterruptedException, ExecutionException {
                return IoFutureBase.this.get();
            }

            public R get(long timeout, TimeUnit unit)
                throws InterruptedException, ExecutionException, TimeoutException
            {
                return IoFutureBase.this.get(timeout, unit);
            }

            public boolean isCancelled() {
                return IoFutureBase.this.isCancelled();
            }

            public boolean isDone() {
                return IoFutureBase.this.isDone();
            }
        });
    }
